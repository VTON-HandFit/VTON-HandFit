...
class HandFitPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    Args:
        vae ([`AutoencoderKL`]):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if pose_image:
            ...
            depth_image = self.image_processor.preprocess(depth_image)
            denspose_image = self.image_processor.preprocess(denspose_image)
        mask = np.array(mask)
        mask[mask < 127] = 0
        mask[mask >= 127] = 255
        ...
        ################################################################################################################################################################
        if self.handprocessor is not None:
            mask = torch.tensor([[1,1],[0,0]]).cuda().half()
            mask_bbox = torch.tensor([[1,1],[0,0]]).cuda().half()
            mask_dino = torch.tensor([[1,1],[0,0]]).cuda().half()
            mask_2d = torch.tensor([[1,1],[0,0]]).cuda().half()

            c_struct,c_dino  = self.handprocessor(vertices, mano_6d, image_pos2d, handtype, image_bbox, dino_embedding,  mask, mask_bbox, mask_dino, mask_2d, decoupling=True)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                mask_latents_input = torch.cat([mask_latents] * 2) if self.do_classifier_free_guidance else mask_latents

                # concat latents, image_latents in the channel dimension
                scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                latent_control_model_input = torch.cat([scaled_latent_model_input, vton_latents], dim=1)
                latent_vton_model_input = torch.cat([scaled_latent_model_input, mask_latents_input[:,0:1], vton_latents], dim=1)
                # latent_vton_model_input = torch.cat([scaled_latent_model_input, vton_latents,depth_latents], dim=1)
                if pose_image is not None:
                    # print(self.pose_guider)
                    # pose_fea = self.pose_guider(pose_image.to(device=noise.device, dtype=noise.dtype),
                    #                             # depth_image.to(device=noise.device, dtype=noise.dtype),
                    #                             # mano_vector.to(device=noise.device, dtype=noise.dtype),
                    #                             # handtype.to(device=noise.device, dtype=noise.dtype),
                    
                    #                             )
                    # # # hand_fea = self.hand_guider(#pose_image.to(device=noise.device, dtype=noise.dtype),
                    # # #                             depth_image.to(device=noise.device, dtype=noise.dtype),  
                    # # #                             # mano_vector.to(device=noise.device, dtype=noise.dtype),
                    # # #                             # handtype.to(device=noise.device, dtype=noise.dtype),
                    # # # )
                    # # denspose_fea = self.denspose_guider(denspose_image.to(device=noise.device, dtype=noise.dtype))
                    # # # pose_fea = pose_fea + denspose_fea

                    # # pose_fea = pose_fea + denspose_fea  # + hand_fea
                    # # # pose_fea
                    ...
                    #     )
                    # with autocast():
                    #     control_fea = self.controlnet(
                    #         x=latent_vton_model_input, hint=depth_image.to(device=noise.device, dtype=noise.dtype), timesteps=torch.tensor([t]).to(device=noise.device, dtype=noise.dtype), context=handobj)
                    if self.controlnet is not None:
                        with autocast():
                            control_fea = self.controlnet( 
                                x=latent_control_model_input,  
                                image_denspose=denspose_image.to(device=noise.device, dtype=noise.dtype), 
                                image_dwpose=pose_image.to(device=noise.device, dtype=noise.dtype), 
                                image_depth=depth_image.to(device=noise.device, dtype=noise.dtype), 
                                timesteps=torch.tensor([t]).to(device=noise.device, dtype=noise.dtype), 
                                # context = prompt_embeds.to(device=noise.device, dtype=noise.dtype),
                                context=c_struct,
                                bbox = image_bbox,
                                )
                    # import pdb;pdb.set_trace()
                    # stacked_tensors = torch.stack(control_fea)
                ...
                noise_pred = self.unet_vton(
                    ...
                    encoder_hidden_states=prompt_embeds,
                    return_dict=False,
                    down_block_additional_residuals = control_fea[:-1],
                    mid_block_additional_residual = control_fea[-1],
                    # pose_cond_fea=pose_fea,
                    # cross_attention_kwargs = {'gligen': handobj},
                    cross_attention_kwargs = {
                        'bbox': image_bbox,
                        'ip_hidden_states': c_dino
                    },
                    # pose_cond_fea=None,
                )[0]
                # else:
    ...
    def _encode_prompt(
        ...
        prompt_embeds = prompt_embeds.to(dtype=self.vae.dtype, device=device)
